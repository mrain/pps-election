# -*- coding: utf-8 -*-
"""2party3elector243districts.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1sSB_Cg-NNqCSR8uQY8iCQk1ySijxMDfe
"""
...
#             delta = 10. * (poly1_ratio < thresh) + 10. * (poly2_ratio < thresh)
            delta = 100. * (poly1_ratio <= 0.75) + 100. * (poly2_ratio <= 0.75)
            delta += 50. * (poly1_ratio <= 0.50) + 50. * (poly2_ratio <= 0.50)
            delta += 25. * (poly1_ratio <= 0.25) + 25. * (poly2_ratio <= 0.25)
        ...
        deltas = 100. * (ratios1 <= 0.7505) + 100. * (ratios2 <= 0.7505)
        deltas += 50. * (ratios1 <= 0.5005) + 50. * (ratios2 <= 0.5005)
        deltas += 20. * (ratios1 <= 0.2505) + 20. * (ratios2 <= 0.2505)
...
def split_polygon(xp, theta, polygon):
    ...
    """ Has form x = xp """
    liney = LineString([[xp, y_min-10], [xp, y_max+10]] @ R)
...
for mmap, name in zip(LOADED_MAPS, [name.split("/")[-1].split(".")[0] for name in MAP_LIST]):
    ...
    for MYPARTY in range(num_parties):
        ...
        try:
            ...
            output_name = f"results/81districts_3electors/{name}_{num_districts}districts_party{MYPARTY+1}.dat"
