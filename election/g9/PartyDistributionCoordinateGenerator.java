 ...
 *
 */
public class PartyDistributionCoordinateGenerator {
	
	private static String file = "<Path to party distribution file>"; // maps/g9/sri_lankan_parliment_resized.png
	private static int Y_MAX = 866;
	private static int X_MAX = 1000;

	private static Color blue = new Color(153, 204, 255);
	private static Color green = new Color(102, 255, 102);
	...
	public static void main(String[] args) throws IOException {
        ...
        for(int i = 1; i < rotatedMat.rows(); i ++) {
			...
			for(int k = 10; k < values.size() - 10; k++) {
				if(values.size() != 0) {
        ...
        Imgcodecs.imwrite("/home/shirish/Desktop/Semester-1/PPS/Project-2/raw.jpg", rotatedMat);
        Imgcodecs.imwrite("/home/shirish/Desktop/Semester-1/PPS/Project-2/result.jpg", result);
    }
	...
	public static double[] getNearestColor(double[] colorValues) {
		...
		double distance = 0;
		double rgbDistanceBlue = 0, rgbDistanceGreen = 0, rgbDistanceYellow = 0;
		
		// https://stackoverflow.com/questions/6334311/whats-the-best-way-to-round-a-color-object-to-the-nearest-color-constant
		rgbDistanceBlue = colorDistance(color, blue);
		rgbDistanceGreen = colorDistance(color, green);
		rgbDistanceYellow= colorDistance(color, yellow);
		
		if(rgbDistanceBlue < rgbDistanceGreen) {
			finalColor = blue;
			distance = rgbDistanceBlue;
		}else {
			finalColor = green;
			rgbDistanceBlue = rgbDistanceGreen;
		}
		
		if(rgbDistanceYellow < distance) {
			finalColor = yellow;
		}
